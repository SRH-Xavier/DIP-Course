...
int dftDemo() {
	...
	cv::Mat srcMat = imread("D:/DIP/OpenCV/Project/FFT/FFT/testImages/rose.jpg", 0);
	cv::Mat magMat;
...
{
	cv::Mat dst;
	cv::Mat src = imread("D:/DIP/OpenCV/Project/FFT/FFT/testImages/rose.jpg", 0);
...
int mouseROI()
{
	cv::Mat srcMat = imread("D:/DIP/OpenCV/Project/FFT/FFT/testImages/rose.jpg");
...
int ifftDemo()
{
	...
	cv::Mat src = imread("0.jpg", 0);
	cv::Mat src2 = imread("1.jpg", 0);
	int m = getOptimalDFTSize(src.rows); //2,3,5的倍数有更高效率的傅里叶变换
	int n = getOptimalDFTSize(src.cols);
	Mat padded, padded2;
	//把灰度图像放在左上角,在右边和下边扩展图像,扩展部分填充为0;
